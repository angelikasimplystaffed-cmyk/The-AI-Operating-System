  ...
  try {
    ...
    if (sheet.getLastRow() === 0) {
      sheet.appendRow([
        ...
        'Tags', 'Unsubscribed', 'Reply Count', 'Last Reply Date', 'Last Reply Snippet'
      ]);
...
// ═══════════════════════════════════════════════════════════════
// checkReplies — run on a timer to log replies into the Sheet
// ═══════════════════════════════════════════════════════════════
function checkReplies() {
  ...
  // Build a map of email → row index (column C = index 2)
  var emailToRow = {};
  for (var i = 1; i < data.length; i++) {
    var rowEmail = data[i][2];
    if (rowEmail) { emailToRow[rowEmail.toLowerCase()] = i + 1; } // 1-based row
  }

  // Search Gmail for unread emails in inbox from the last 7 days
  // that are NOT from our own address (i.e. they are replies from leads)
  var threads = GmailApp.search('in:inbox is:unread newer_than:7d -from:me', 0, 50);
  ...
  for (var t = 0; t < threads.length; t++) {
    ...
    for (var m = 0; m < messages.length; m++) {
      ...
      var snippet = msg.getPlainBody().substring(0, 200); // first 200 chars
      var date    = msg.getDate().toLocaleString('en-GB', { timeZone: 'Europe/London' });

      // Extract email address from "Name <email@domain.com>" format
      var match     = from.match(/<(.+?)>/) || from.match(/([^\s]+@[^\s]+)/);
      var fromEmail = match ? match[1].toLowerCase() : from.toLowerCase();

      // Check if this sender is a lead in our sheet
      if (emailToRow[fromEmail]) {
        var rowNum      = emailToRow[fromEmail];
        var currentCount = data[rowNum - 1][9] || 0; // Column J = index 9 = Reply Count

        // Update columns J (Reply Count), K (Last Reply Date), L (Last Reply Snippet)
        sheet.getRange(rowNum, 10).setValue(Number(currentCount) + 1);
        sheet.getRange(rowNum, 11).setValue(date);
...
// ═══════════════════════════════════════════════════════════════
// createReplyTrigger — run ONCE manually to set up auto-checking
// ═══════════════════════════════════════════════════════════════
function createReplyTrigger() {
  // Delete any existing triggers for checkReplies first
  var triggers = ScriptApp.getProjectTriggers();
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'checkReplies') {
      ScriptApp.deleteTrigger(triggers[i]);
    }
  }
  // Create a new trigger: run checkReplies every 10 minutes
  ScriptApp.newTrigger('checkReplies')
    .timeBased()
    .everyMinutes(10)
    .create();
  Logger.log('Trigger created: checkReplies runs every 10 minutes');
}
